...
function parseRegExp(str) {
    ...
    while (str.length > 0) {
        const sequence = str |> parseSequence;

        if (!sequence) return null;

        const normalObjects = sequence.split('').map(char => new Normal(char));
        regexpList.push(...normalObjects);
        str = str.slice(sequence.length);

        if (str[0] === '(') {
            const endIndex = findMatchingClosingParenIndex(str);
            if (!endIndex) return null;
            const innerStr = str.slice(1, endIndex);
            regexpList.push(parseRegExp(innerStr));
            str = str.slice(endIndex + 1);

        } else if (str[0] === '|') {
            if (regexpList.length === 0) return null;
            const left = regexpList.length > 1 ? new Str([...regexpList]) : regexpList.pop();
            regexpList.length = 0;
            const right = parseRegExp(str.slice(1));
            if (!right) return null;
            regexpList.push(new Or(left, right));
            break;

        } else if (str[0] === '*') {
            const prevRegExp = regexpList.pop();
